...
let endpoint =
  "http://content.guardianapis.com/search?format=json&from-date=" +
  today_date +
  "&order-by=newest&show-blocks=all&api-key=" +
  subscriptionKey;
let endpoint2 =
  "http://content.guardianapis.com/search?q=world&format=json&from-date=" +
  week_date +
  "&show-blocks=all&api-key=" +
  subscriptionKey;
      ...
      request(request_params, async function (error, response, body) {
        if (error) {
          ...
          for (let i = 0; i < body.response.results.length; i += 1) {
            if (
              body.response.results[i].webTitle !== "" &&
